    ...
    match st {
                    ...
                    for state_or_def in list {
                        match state_or_def {
                            ast::BlockItem::Declaration(decl) => {
                                out.push((pretty_decl(decl), deep))
                            }
        ...
        ast::Statement::ForDecl {
            ...
            out.push((
                format!(
                    "exp: {}",
                    exp3.as_ref().map_or("".to_string(), |e| pretty_expr(&e))
                ),
                deep,
            ));
            out.push(("DO".to_string(), deep));
        ...
        ast::Statement::Break => vec![("BREAK".to_string(), deep)],
        ast::Statement::Continue => vec![("CONTINUE".to_string(), deep)],
        _ => unimplemented!(),
    }
}
...
fn pretty_opt_expr(exp: &Option<ast::Exp>) -> String {
    exp.as_ref()
        .map_or("None".to_owned(), |exp| pretty_expr(exp))
}

fn pretty_expr(exp: &ast::Exp) -> String {
    match exp {
        ast::Exp::BinOp(op, exp1, exp2) => format!(
            "{} BIN_OP<{:?}> {}",
            pretty_expr(exp1),
            op,
            pretty_expr(exp2)
        ...
        ast::Exp::AssignOp(name, op, exp) => {
            format!("VAR[{}] ASSIGN_OP<{:?}> {}", name, op, pretty_expr(exp))
        }
        ast::Exp::CondExp(cond, exp1, exp2) => format!(
            "TERNAR_OP IF {} DO {} ELSE DO {}",
            pretty_expr(cond),
            pretty_expr(exp1),
            pretty_expr(exp2)
        ),
        ast::Exp::FuncCall(name, params) => format!(
            "CALL {} WITH {}",
            name,
            params
                .iter()
